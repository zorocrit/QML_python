...
class QSVM_routine:
    def __init__(self, feature_dim = 2, feature_depth = 2, train_test_split = 0.3, train_samples = 5, test_samples = 2, seed = 0, copies = 5):
    ...
    def train_test_datasets(self):
        ...
        self.test_X_norm = self.z_scale.transform(text_X)
        ...
        def train_model(self):
            ...
            feature_expansion = SecondOrderExpansion(feature_dimension = self.feature_dim, depth = self.feature_depth, entangler_map = [[0, 1]])
            # Model definition
        ...
        def analyze_training_and_inference(self, result, svm):
            ...
            image = plt.imshow(np.asmatrix(data_kernel_matrix),interpolation='nearest', origin='upper', cmap='bone_r')
            plt.show()
